...
// ── DR-044: bookingRef là object { key }, không phải chuỗi — PY3/PY4/PY5 phải đọc bookingRef.key ──
...
test('DR-044: bookingRef object không có key (hoặc key rỗng) → coi như không có bookingRef, không nổ', () => {
  const prices = map({ 'tour-a': { unit: 'perPax', amount: 500000 } })
  const docs = [
