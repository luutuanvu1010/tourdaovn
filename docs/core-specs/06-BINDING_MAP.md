...
- **Phiên bản:** v2.14.0   **Trạng thái:** v2.0.0 là bản mở cổng bước 7 ngày 2026-08-06 (QĐ-2026-08-06-06); v2.1.0 (đợt 4B) duyệt 2026-08-22, QĐ-2026-08-22-02; **v2.2.0 duyệt 2026-08-22, QĐ-2026-08-22-05** — đổi ở §3, §3.1, §6, §4.8, §4.10, §7 và §7.1 (sửa theo N18: bản đầu của dòng này khai thiếu bốn mục sau); **v2.3.0 duyệt 2026-08-24, QĐ-2026-08-23-02** — đoạn mở rời hero (§3, §3.1) và Luật 5 cột→hàng ở di động (§6); **v2.3.1 duyệt 2026-08-24, QĐ-2026-08-24-01** — Điểm tham quan miễn phí không còn vùng giá, nhãn "Miễn phí" chỉ ở Thông tin nhanh (§3.1); **v2.4.0 duyệt 2026-08-24, QĐ-2026-08-24-05** — `title` rời lớp phủ hero xuống dải sáng riêng ngay dưới ảnh, TRƯỚC thanh dính; hero chỉ còn huy hiệu loại và ảnh (§3); **v2.5.0 duyệt 2026-08-25, QĐ-2026-08-25-01** — breadcrumb và dải tiêu đề chuyển lên TRÊN hero; hero rút còn 380px desktop / 240px di động; hero **ẩn hẳn** khi không ảnh và không huy hiệu (§3); **v2.6.0 duyệt 2026-08-25, QĐ-2026-08-25-02** — bỏ luật "≤ 2 ô thì không trải dải"; `author` chốt đúng một vùng là Hộp tác giả (§3); **v2.7.0 duyệt 2026-08-28, QĐ-2026-08-28-01** — **xoá hàng "Trust bar" khỏi §4.1** và khỏi danh sách khối của §5.7. Hàng đó khai `config (build)` nhưng đòi *"cam kết hệ thống … không phải CTA marketing"*, trong khi nguồn duy nhất có thật (`HOME_COPY.trustItems`) là bốn lời hứa bán tour của §5.7. Đo trên bản dựng 2026-08-28: cả bốn trang điểm đến hiện khối bán hàng, và **không trang nào** hiện được ba cam kết mà hàng này gọi tên. Vùng bị gỡ hẳn thay vì để một hàng "bắt buộc" chưa bao giờ được thi hành; xem `ADR-0029`; **v2.8.0 duyệt 2026-08-28, QĐ-2026-08-28-02** — hero thêm biến thể `tall` (+50px) chỉ cho trang điểm đến (§3 hàng Hero); **v2.9.0 duyệt 2026-08-28, QĐ-2026-08-28-03** — gộp hai biến thể hero làm MỘT chiều cao cho mọi trang (430/390/290), kèm **ngoại lệ Luật 3 có ghi phiếu** (§3 hàng Hero); **v2.10.0 duyệt 2026-08-29, QĐ-2026-08-29-02** — Bài viết chuyển từ `InfoCard` sang `FactStrip`, vào đúng khung chung; khai vùng ba field của Bài viết ở đầu §3.1; **v2.11.0 duyệt 2026-08-29, QĐ-2026-08-29-03** — Tổ chức và Người chuyển nốt, `DR-046` đóng, `InfoCard.astro` xoá; **v2.12.0 duyệt 2026-08-29, QĐ-2026-08-29-04** — tách `PageHead` làm đầu trang dùng chung cho 6 loại trang (trừ trang chủ); Bài viết đưa chuyên mục/tác giả/ngày lên byline không nền, `author` đổi vùng từ Hộp tác giả sang byline; **v2.13.0 duyệt 2026-08-29, QĐ-2026-08-29-05** — `article` gỡ khỏi danh sách loại trừ ở §3 hàng Gallery; Bài viết có `gallery`, hero vào được mosaic; **§4.10 sửa 2026-08-30** — dòng mở của §4.10 còn giữ nguyên câu "Không gallery" của bản v2.12, tức cùng một bản duyệt khai hai điều trái nhau ở hai mục. Sửa cho khớp §3, KHÔNG phải quyết định mới; **v2.14.0 duyệt 2026-08-31, QĐ-2026-08-31-03** — nới Luật 3, xoá điều kiện "0 trang" của ngoại lệ hero (§3 hàng Hero)
...
|---|---|---|---|---|
| Breadcrumb (điều hướng) | `config (build)`: nhánh URL theo 05 mục 1.2, rồi `title` của trang | có, **mọi** trang chi tiết | không có trạng thái rỗng — chuỗi tối thiểu là *Trang chủ → nhánh → tiêu đề* | **tách ra ở v2.2 (N3).** Đây là **vùng điều hướng**, sinh từ cây URL chứ không từ field entity, nên không entity nào miễn. Nằm trên dải sáng **phía trên** hero, không đè lên ảnh. **v2.5: mã đã theo.** Từ v2.2 tới 2026-08-25 `DetailLayout` vẫn đặt Breadcrumb trong `slot="overlay"` của Hero, tức đè lên ảnh và phải bật `inverse` — lệch đặc tả suốt ba ngày mà không cổng nào bắt. `QĐ-2026-08-25-01` dựng `.crumb-band` riêng và bỏ `inverse`. Nuôi `BreadcrumbList` trong JSON-LD |
...
| Hero | **v2.5: trên ảnh chỉ còn huy hiệu loại.** `title` và `summary` đều đã ra ngoài (dải tiêu đề ở TRÊN hero, dải đoạn mở ở dưới thanh dính) — **không còn chữ nào đè lên ảnh**. Hero nuôi bằng `mainImage` (kèm alt); `gallery` đủ 4 ảnh sau khi loại trùng `mainImage` thì đi qua Hero mosaic. **Chiều cao (v2.5):** desktop `clamp(280px, 30vw, 380px)` (cả biến thể mosaic lẫn ảnh đơn; 340px ở 769–1023px), di động 240px — rút từ 430/400/280 để giữ **Luật 3** sau khi hai dải chữ lên trên. **ĐÃ ĐO trên bản dựng, Chrome headless qua CDP, 2026-08-25.** Không còn là ước lượng. Ở viewport **1366×657** (chiều cao thật của trình duyệt trên màn 768, sau khi trừ chrome): tiêu đề **1 dòng** (`tour/ve-vin-harbour`, 14 ký tự) → thanh dính **618→675px**, mép trên lọt màn đầu, mép dưới bị cắt; tiêu đề **2 dòng** (`tour/hon-tam-tron-goi…`, 65 ký tự) → thanh dính **674→731px**, **nằm hẳn ngoài màn đầu**. Ở viewport 1366×768 (không trừ chrome) thì cả hai đều lọt. **Kết luận: với tiêu đề dài, thanh dính KHÔNG còn trên màn đầu ở laptop phổ biến.** **v2.9.0 (`QĐ-2026-08-28-03`): MỘT chiều cao duy nhất cho mọi trang** — `clamp(380px, calc(30vw + 100px), 480px)` desktop, **440px** ở 769–1023px, **340px** di động. Áp cho **cả trang chi tiết entity lẫn trang điểm đến**, không còn biến thể. (v2.8.0 từng dựng biến thể `tall` chỉ cho trang điểm đến; chủ dự án quyết mọi trang chi tiết entity dùng cùng một định dạng hero, nên hai giá trị gộp làm một — một biến thể mà mọi nơi gọi đều bật thì không còn là biến thể.) ⚠ **NGOẠI LỆ LUẬT 3 CÓ GHI PHIẾU — `QĐ-2026-08-31-03` (2026-08-31).** Điều kiện "0 trang" của `QĐ-2026-08-28-03` **đã hết hiệu lực và bị xoá**: giá render thật trên thanh dính từ 2026-08-31. Chủ dự án đã xét lại và **chấp nhận** thanh dính mang giá không còn trên màn đầu ở 1366 (đo **≈718px** so với mốc **657px** — số **suy ra**: 668px kế thừa + 50px theo lập luận clamp, KHÔNG đo lại được ở 1366 trong phiên này, xem `QĐ-2026-08-31-03`), đổi lấy hero cao thêm 50px. Đây là vi phạm Luật 3 **được chấp nhận có chủ ý**, không phải "chưa thành vi phạm". ⚠ Con số +50px phải cộng vào **cả năm giá trị** của `clamp`, không chỉ số giữa: ở dải ≥1267px (gồm cả 1280 và 1366) thì **trần** mới là số đang trói, không phải số giữa — số giữa chỉ trói ở dải 1024–1266px. Nâng riêng trần **480 lên cao hơn** (giữ nguyên số giữa `calc(30vw + 100px)`) chỉ cho +4px ở 1280 và +30px ở 1366.** | mainImage nên có | **v2.5: không ảnh VÀ không huy hiệu → ẩn hẳn hero**, không dựng khung. Trước v2.5 ô này ghi "hero thuần chữ", nhưng từ khi `title`/`summary` ra ngoài thì biến thể không-ảnh không còn chữ nào — nó thành khối gradient rỗng, vi phạm **R7**. Có huy hiệu mà không ảnh thì vẫn dựng hero gradient. Gallery dưới 4 ảnh thì hero ảnh đơn; không ảnh placeholder | desktop đủ 4 gallery: ảnh chính bên trái, gallery 2x2 bên phải kiểu Hotel với 4 ô bằng nhau; mobile: ảnh chính trên, thumbnail strip 4 ô đều dưới; alt bắt buộc khi có ảnh (2.0) |
| Dải tiêu đề | `title` | có (gate) | — | **v2.5 (`QĐ-2026-08-25-01`):** dải sáng riêng ngay **TRÊN** hero, dưới breadcrumb. (v2.4 đặt nó dưới hero; v2.5 đưa lên trên theo yêu cầu chủ dự án, đổi lại phải rút hero 430→380px để thanh giá không rơi khỏi màn đầu.) Lý do rời khỏi lớp phủ: chữ trên ảnh buộc phải canh tương phản theo từng tấm — QA1 đợt 4B đo 6,6:1 ở bộ `ngoc-lam`; trên nền sáng cặp `--c-text`/`--c-surface` đo **17,85:1** (`bien-sau`) và **16,80:1** (`cat-bien`). Đứng **trước** thanh dính chứ không sau: đặt sau sẽ đẩy thanh giá xuống thêm ~152px trên desktop, rơi khỏi màn đầu ở 1366×768 — vướng **Luật 3**. Mã: `DetailLayout.astro`, `.title-band`. **Chưa gắn `data-region`** — `title` không có trong ma trận §3.1 nên `luat1-post` chưa canh vùng này. Gắn nó là việc riêng cần phiếu, vì `title` còn xuất hiện ở **mắt cuối breadcrumb**: đưa `title` vào §3.1 mà không khai ngoại lệ cho breadcrumb sẽ làm Luật 1 báo lặp vùng. Breadcrumb sinh từ cây URL chứ không từ field entity (§3 hàng Breadcrumb), nên ngoại lệ là hợp lý — nhưng phải viết thành chữ trước khi bật cổng |
| Đoạn mở | `summary` | có (gate, I10) | — | **v2.3:** nằm trên **dải sáng dưới hero, sau thanh dính**, trên Thông tin nhanh — **không còn đè lên ảnh hero**. Tự đứng được như câu trả lời hoàn chỉnh; nguồn speakable. Lý do đổi: chữ trên ảnh buộc phải canh lớp phủ theo từng ảnh (QA1 đợt 4B đo 6,6:1 ở bộ `ngoc-lam`); đưa xuống dải sáng thì rủi ro tương phản biến mất |
...
| Gallery | `gallery` (kèm alt từng ảnh) | nên có với entity có field | ẩn | Không render gallery section rời trên detail; gallery detail phải đi qua Hero mosaic. **v2.13.0 (`QĐ-2026-08-29-05`): `article` GỠ khỏi danh sách loại trừ** — Bài viết nay có field `gallery` trong schema, truy vấn và kiểu, nên hero của nó vào được mosaic như mọi entity khác. Trước đó trang cẩm nang luôn rơi về hero ảnh đơn, đúng đặc tả cũ chứ không phải lỗi. ⚠ Đổi mã KHÔNG tự làm trang đổi hình: mosaic cần **đủ 4 ảnh** sau khi loại trùng `mainImage`, nên mỗi bài phải được nạp ảnh thì mới lên lưới; chưa nạp thì fallback một ảnh (`QĐ-2026-08-28-03`). không áp dụng: person, organization |
...
| Thông tin nhanh | các field ngắn của entity theo ma trận §3.1 — giờ mở cửa, địa chỉ, điện thoại, website chính thức, "miễn phí" khi đúng, thời lượng, phù hợp với, khởi hành | tùy | ô rỗng không render; 0 ô thì không có vùng | **thêm v2.1**, thay cặp InfoBar + InfoCard. Mỗi field **đúng một ô**, tối đa 6 ô, icon SVG, **không chứa giá**; **v2.6 (`QĐ-2026-08-25-02`): BỎ luật "≤ 2 ô thì không trải dải".** Thông tin nhanh **luôn trải dải**, mọi trang, mọi số ô. Luật cũ cắt theo *số lượng dữ liệu* chứ không theo loại trang, nên cùng một entity mà trang 2 ô đẩy vùng này vào cột phụ còn trang 3 ô trải dải — đo được: `trai-nghiem/cano-keo-du-bay` và `phao-chuoi` đều 2 ô nên vào cột phụ, `diem-tham-quan/chua-long-son` 5 ô và `tour/ve-vin-harbour` 3 ô thì trải dải. Một wireframe ra hai hình dạng tuỳ dữ liệu là thứ chủ dự án gọi tên ở lượt rà 2026-08-25. **di động theo Luật 5 §6: mỗi ô một hàng** (v2.3, thay câu "giữ lưới 2 cột" của v2.1). Desktop: dưới thanh dính; di động: ngay dưới tiêu đề. Component `FactStrip.astro` |
...
**Bài viết (v2.12.0, `QĐ-2026-08-29-04`).** Ba field `articleType`, `publishedAt` và `author` chuyển lên **BYLINE** — một dòng chữ nhỏ ngay dưới `h1`, không nền không viền, dựng bởi `PageHead` qua slot `meta`. Chủ dự án chốt 2026-08-29: ba mẩu thông tin ngắn này không đáng chiếm hai dải riêng. `author` vì vậy đổi vùng duy nhất từ "Hộp tác giả" sang **byline**, và nay CÓ link `/tac-gia/{slug}` — thứ §3 đã đòi mà bản cũ chưa làm. `updatedAt` **không** lên byline: `DetailLayout` đã hiện "Cập nhật" ở cuối nội dung, đưa lên nữa là hai vùng cho một ý. Bài viết do đó không còn ô Thông tin nhanh nào — `FactStrip` tự ẩn (R7).

*Đoạn dưới đây là bản v2.10.0, giữ để đọc lịch sử:*

**Bài viết (v2.10.0, `QĐ-2026-08-29-02`).** Bài viết không có cột riêng trong ma trận bốn cột dưới đây — bốn cột đó là bốn entity có hợp đồng đầy đủ. Nhưng từ v2.10.0 Bài viết **dùng chung `FactStrip`** như chín entity kia, nên phải khai vùng ở đây thay vì để trống: `articleType` → **Thông tin nhanh** (không còn là huy hiệu hero thứ hai; huy hiệu hero vẫn giữ, xem §4.10), `publishedAt` → **Thông tin nhanh**, `updatedAt` → **Thông tin nhanh**. `author` giữ nguyên đúng một vùng là **Hộp tác giả** (v2.6.0). Trước v2.10.0 ba field này nằm trong `InfoCard` ở cột phụ — vùng riêng của ba template ngoài phạm vi đóng Luật 1, xem `DR-046`.

**Tổ chức và Người (v2.11.0, `QĐ-2026-08-29-03`).** Hai template cuối cũng chuyển sang `FactStrip`, `DR-046` đóng hẳn. Vùng: `orgType`/`jobTitle`, `address`, `telephone`, `url`, `officialSource`, `licenseInfo`, `knowsAbout` → **Thông tin nhanh**. `sameAs` **KHÔNG** vào Thông tin nhanh — giữ đúng hàng §3.1 ở trên: dòng "Nguồn tham khảo" cạnh Cập nhật, nay truyền qua prop `sameAs` của `DetailLayout`.
...
| `summary` | dải đoạn mở | dải đoạn mở | dải đoạn mở | dải đoạn mở |
...
**Có `gallery`** từ `QĐ-2026-08-29-05` — nuôi Hero mosaic như mọi entity khác; ảnh inline trong body (2.11) vẫn giữ, hai thứ khác vai. Dòng này trước ghi "Không gallery", trái với chính §3 hàng Gallery cùng bản v2.13.0; sửa 2026-08-30. Document-level: mỗi ngôn ngữ một document, switcher theo translationGroup.

| Vùng giao diện | Dữ liệu nuôi | Bắt buộc? | Khi rỗng thì hiện gì | Ghi chú |
|---|---|---|---|---|
| Nhãn chuyên mục | `articleType` | có (gate) | — | xuất articleSection trong JSON-LD |
| Hộp tác giả | `author` deref Person: title, `mainImage`, `summary` | có (gate I4) | — | link `/tac-gia/{slug}`, tín hiệu E-E-A-T. **v2.6 (`QĐ-2026-08-25-02`): đây là vùng DUY NHẤT của `author`.** Trước đó `ArticleDetail` còn phát tên tác giả ở hai chỗ nữa — phù hiệu hero "Bởi {tên}" và một hàng InfoCard ở sidebar — tức **ba vùng cho một field**, vi phạm Luật 1 trên mọi trang cẩm nang. Cổng không bắt được vì chỉ một trong ba chỗ gắn `data-field` (khoảng mù `DR-048`). Hai chỗ thừa đã gỡ; hộp này nay mang `data-region="author-box"` và `data-field="author"`. ⚠ Thẻ đó **chưa** làm cổng canh được: tầng B của `luat1-post` xét theo bốn cột entity của §3.1, mà Cẩm nang không có cột — thêm cột là việc riêng, cần phiếu |
...
| Các khối nội dung | như §4.1 (banner, hub grid, areas, guides, featured, faq, safety) | tùy | khối rỗng tự ẩn dù `hidden = false` | empty guard là cổng cứng, xem §4.1 |
...
- **Luật 5 (v2.3) — ô cạnh nhau trên desktop, mỗi ô một hàng trên di động.** Mọi vùng trình bày nhiều ô ngang hàng ở desktop — Thông tin nhanh, khối hành động, dòng meta, thẻ thông tin — ở `≤ 640px` chuyển thành **mỗi ô một hàng chiếm hết bề rộng**. Cấm bóp cột: không ô nào được hẹp hơn 100% bề rộng vùng chứa.
  - **Trong một hàng, chọn kiểu theo độ dài giá trị.** Giá trị vừa một dòng thì **nhãn trái, giá trị phải** — gọn và dễ quét. Giá trị dài quá một dòng thì **nhãn trên, giá trị full-width bên dưới**, vì chia đôi hàng sẽ bóp giá trị lại đúng thứ luật này định gỡ.
  - **Căn cứ đo được (2026-08-24, 272 ô trên bản dựng thật):** độ dài trung vị **13 ký tự**; **75/272 ô — 28% — dài quá 20 ký tự**; dài nhất là `address` **78 ký tự** và `touristType` 44. Ở 390px, lưới 2 cột cho mỗi ô ~170px ≈ 20–24 ký tự một dòng — nên 72% số ô vừa khít, và luật này tồn tại vì 28% còn lại. Chia đôi hàng chỉ cho giá trị ~190–215px, hơn lưới 2 cột chừng 15–25%; nhãn-trên mới cho trọn ~358px.
  - **Loại trừ: lưới thẻ không thuộc luật này.** Thẻ Tour, Gần đây, Trải nghiệm tại đây mang ảnh và tiêu đề, không phải cặp nhãn/giá trị. Chúng xếp chồng dọc, mỗi thẻ một khối. Luật 5 chỉ áp cho vùng dữ liệu dạng **nhãn/giá trị**.
  - **⚠ Luật này THAY câu "di động giữ lưới 2 cột" ở hàng "Thông tin nhanh" của §3.** Hai câu không tương thích: lưới 2 cột giữ hai ô cạnh nhau, Luật 5 tách mỗi ô một hàng. Hàng §3 đã sửa theo. **Hệ quả lên mã đang chạy:** `FactStrip.astro` vừa được sửa (2026-08-23) để theo đúng câu cũ; áp Luật 5 là sửa lại CSS của đúng file đó.
