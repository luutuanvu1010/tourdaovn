...
| Hero | **v2.5: trên ảnh chỉ còn huy hiệu loại.** `title` và `summary` đều đã ra ngoài (dải tiêu đề ở TRÊN hero, dải đoạn mở ở dưới thanh dính) — **không còn chữ nào đè lên ảnh**. Hero nuôi bằng `mainImage` (kèm alt); `gallery` đủ 4 ảnh sau khi loại trùng `mainImage` thì đi qua Hero mosaic. **Chiều cao (v2.5):** desktop `clamp(280px, 30vw, 380px)` (cả biến thể mosaic lẫn ảnh đơn; 340px ở 769–1023px), di động 240px — rút từ 430/400/280 để giữ **Luật 3** sau khi hai dải chữ lên trên. **ĐÃ ĐO trên bản dựng, Chrome headless qua CDP, 2026-08-25.** Không còn là ước lượng. Ở viewport **1366×657** (chiều cao thật của trình duyệt trên màn 768, sau khi trừ chrome): tiêu đề **1 dòng** (`tour/ve-vin-harbour`, 14 ký tự) → thanh dính **618→675px**, mép trên lọt màn đầu, mép dưới bị cắt; tiêu đề **2 dòng** (`tour/hon-tam-tron-goi…`, 65 ký tự) → thanh dính **674→731px**, **nằm hẳn ngoài màn đầu**. Ở viewport 1366×768 (không trừ chrome) thì cả hai đều lọt. **Kết luận: với tiêu đề dài, thanh dính KHÔNG còn trên màn đầu ở laptop phổ biến.** **v2.8.0 (`QĐ-2026-08-28-02`): thêm biến thể `tall`, cao thêm 50px ở mọi khổ — `clamp(330px, calc(30vw + 50px), 430px)` desktop, 390px ở 769–1023px, 290px di động. **CHỈ trang điểm đến (§4.1) được bật**; trang chi tiết giữ nguyên 380/340/240. Lý do là chính đoạn trên: đã đo ở 1366, `tall` đẩy thanh dính từ 618 xuống 668, vượt mốc màn đầu 657px — tức giá rơi khỏi màn đầu với MỌI tiêu đề chứ không riêng tiêu đề hai dòng, đúng thứ `QĐ-2026-08-25-01` đã cắt 430→380 để tránh. Trang điểm đến không có thanh dính nên Luật 3 không trói ở đó. ⚠ Con số phải cộng vào **số giữa** của `clamp`: nâng riêng trần 380→430 chỉ cho +4px ở 1280 và +30px ở 1366, vì `30vw` mới là số đang trói.** Hôm nay chưa thành vi phạm Luật 3 vì `sticky-bar__price` render trên **0 trang** — thanh chỉ mang CTA; nhưng ngày giá chạy được thì nó vi phạm. | mainImage nên có | **v2.5: không ảnh VÀ không huy hiệu → ẩn hẳn hero**, không dựng khung. Trước v2.5 ô này ghi "hero thuần chữ", nhưng từ khi `title`/`summary` ra ngoài thì biến thể không-ảnh không còn chữ nào — nó thành khối gradient rỗng, vi phạm **R7**. Có huy hiệu mà không ảnh thì vẫn dựng hero gradient. Gallery dưới 4 ảnh thì hero ảnh đơn; không ảnh placeholder | desktop đủ 4 gallery: ảnh chính bên trái, gallery 2x2 bên phải kiểu Hotel với 4 ô bằng nhau; mobile: ảnh chính trên, thumbnail strip 4 ô đều dưới; alt bắt buộc khi có ảnh (2.0) |
