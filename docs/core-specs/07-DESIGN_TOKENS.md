...
| layout.hero.entity.max | 480px | **trần** chiều cao hero của trang chi tiết entity và trang điểm đến. Nguồn duy nhất là `--hero-entity-h-max` trong `tokens.css`; `Hero.astro` không giữ con số nào. Đổi ở đây là mọi loại trang đổi theo, cả ba biến thể hero (mosaic, ảnh đơn, không-ảnh). ⚠ Khác `--hero-min-h` — token đó của `HomeHero`, hero **trang chủ** |
| layout.hero.entity.min | 380px | **sàn** của cùng biểu thức. Số giữa là `calc(30vw + 100px)`, không phải `30vw` — nâng riêng trần chỉ cho +4px ở 1280 và +30px ở 1366 (`QĐ-2026-08-31-03`) |
| layout.hero.entity.tablet | 440px | khoảng 769–1023px, giá trị phẳng |
| layout.hero.entity.mobile | 340px | ≤768px, giá trị phẳng |
