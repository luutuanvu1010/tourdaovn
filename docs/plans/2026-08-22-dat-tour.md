...
- **Không chạm** `cms/schemas/`, `01-CONTENT_MODEL`, `05-URL_MAP`, `site.config.ts` mảng
  `nav`. Không deploy Studio — duy nhất Task 12 chạy một script `sanity exec` patch 8 document
...
2. Trong Sanity, **8 tour approved đang chứa CHUỖI GIÁ trong `bookingRef.key`** (ví dụ
   ...
   `bookingRef`. Đây là **drift** phải ghi `DRIFT_LOG` (Task 1) và phải chuyển (Task 12) thì
...
| `src/components/TourDetail.astro`, `DetailLayout.astro`, `Sidebar.astro` | nối form vào trang tour, thanh dính trỏ `#dat-tour` | 11 |
| `cms/_migrate-bookingref-keys.mjs`, `data/prices.yaml` | chuyển 8 `bookingRef` thành khoá, viết dòng giá thật | 12 |
...
Xử ở kế hoạch `docs/plans/2026-08-22-dat-tour.md` Task 12: khoá = slug tiếng Việt của tour,
con số chuyển sang `data/prices.yaml` (`amount` + `paxRates`). Không sửa mục này; đóng bằng
một dòng "đã xử" khi Task 12 xong.
```
